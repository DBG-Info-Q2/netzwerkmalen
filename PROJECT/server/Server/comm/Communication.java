...
public class Communication
{
    ...
    // Defines whether or not the server should be running. Please use startListener and forceShutdown instead of setting this variable.
    ...
    public void startListener() {
        ...
        if(!serverRunning){
            ...
            try{ 
                SocketServer=new ServerSocket(SERVER_PORT); // Create ServerSocket that handles first incoming connections.
            }catch(IOException e){
            ...
            // Create asynchronous Loop in order to listen to incoming connections without interrupting the gameplay.
            Listener = new Thread(new Runnable(){

                    @Override
                    public void run(){
                        Logger.log("Listening for new connections..");
                        while(serverRunning){ // As long as the server should be running. If for some reason serverRunning gets set to false, Thread will gracefully come to an end.
                            try{
                                Socket newClient = SocketServer.accept(); // Halting listener Thread until new Connection established with client.

                                // Create an Unique ID for the Player.
                                String clientID =PlayerNamesUtil.findNewName(playerList.keySet().toArray(new String[playerList.keySet().size()]),null); // Converts current Player List to Names Array
                                ...
                                // Send out a new login paket to all connected clients in order to update leaderboard / welcome message.
            ...
            // Self-explanitory
            Logger.error("There is already a SocketServer running and listening for incoming connections on the serverside.");
        }
    }
    ...
    /**
     * This procedure handles an incoming Paket from a client with a given id. If the protocol is correct, so the Paket is destin for the server, it gets handled.
     * @author Lord_von_X
     */
    public static void handlePaket(String id, String paket)
    {
        String[]analyse = paket.split(";"); // Split Paket into argument groups. First one is always Paket type. Following are Paket argumentation.
        if(analyse.length==0){
        ...
        switch (analyse[0])
        {
            case "0": Logger.error("False protocol. Client "+id+" sent LoginUpdatePaket."); break; // Paket destin for client
            case "1": Logger.error("False protocol. Client "+id+" sent TimeUpdatePaket."); break; // Paket destin for client
            ...
            if (Eingabekontrolle.checkWord(analyse[1])){
                // Word is contained, calculate points for the user and handle adding them to him.
                ...
                // Paket could be filtered out to not be the word in search, so broadcast chat to all clients.
                // TODO: Alter chat message, so there is the players Name in front of the message.
                sendPaket("-1", paket);
            }
            break;
            case "3": Logger.error("False protocol. Client "+id+" sent PointsUpdatePaket."); break; // Paket destin for client
            case "4":
            // WIP
            //Logger.log("Client "+id+" sent DrawUpdatePaket.");
            sendPaket("-1", paket); 
            break;
            case "5": Logger.error("False protocol. Client "+id+" sent RoleUpdatePaket."); break; // Paket destin for client
            case "6": Logger.error("False protocol. Client "+id+" sent WordUpdatePaket."); break; // Paket destin for client
            case "7": Logger.error("False protocol. Client "+id+" sent GameStateUpdatePaket."); break; // Paket destin for client
            case "8": Logger.error("False protocol. Client "+id+" sent GameEndUpdatePaket."); break; // Paket destin for client
            default:  Logger.error("False protocol. Client "+id+" sent trash."); break; // Bullshit
        }
        // Yep. You have reached the end of our beatiful handler.
    }

    /**
     * This procedure sends out Pakets to a given client or broadcasts it ("-1") to all clients.
     */
    public static void sendPaket(String id, String paket){
        ...
        // Get the COMMSocket for the client in question and use its build in procedure to send the Paket.
